...
class RNN_classifier(nn.Module):
    def __init__(self,hidden,word_size,layers_size,binary = False):
        ...
        self.embedding = nn.Embedding.from_pretrained(text_field.vocab.vectors)
        self.hidden=200
    ...
    def forward(self,input):
        ...
        if self.binary:
            ...
            # because we use CrossEntropyLoss loss function, it already applies log_softmax
...
tag1 = [2 if single >=90  else 1 if single>=85 else 0 for single in score.points]
score['tag'] = tag
score['tag1'] = tag1
plt.figure()
...
test_data.to_csv('/home/max/python/text_mining/project_2/data/test2.csv',index=False)
train_data.to_csv('/home/max/python/text_mining/project_2/data/train2.csv',index=False)
validation_data.to_csv('/home/max/python/text_mining/project_2/data/vali2.csv',index=False)
...
train,vali ,test = TabularDataset.splits(path=folder ,train='train2.csv',validation='vali2.csv' 
...
text_field.build_vocab(train, vectors='glove.6B.200d')

class_weights = torch.tensor(np.array(train_data.tag1.value_counts())/test_data.shape[0])

multi_loss = nn.CrossEntropyLoss(weight=class_weights.to(device).float())

def train(model,optimizer,loss_function=multi_loss,
          train_loader = train_iterator,epochs = 10,
        ...
        for ((description,_),tag,tag1),_ in train_loader:
            ...
            if global_step % validation_cycle==0:
                ...
                if valid_loss < best_loss:
                    ...
                    save_checkpoint('model_multi.pt', model, optimizer, valid_loss)
...
def evaluate(model,test_loader):
  ...
  with torch.no_grad():
      for ((description,_),tag,tag1),_ in test_loader:
          ...
          y_true.extend(tag1.tolist())
    
      
      y_pred = ['High End' if single ==2 else 'Mid Range' if single ==1 else 'Low End' for single in y_pred]
      y_true = ['High End' if single ==2 else 'Mid Range' if single ==1 else 'Low End' for single in y_true]
      print(classification_report(y_true, y_pred, digits=4))
      ef = confusion_matrix(y_true, y_pred)
      ef = pd.DataFrame(ef,index=['High End','Low End' ,'Mid Range'],columns=['High End','Low End' ,'Mid Range'])
      #print(ef)
...
load_checkpoint('model_multi.pt', model, optimizer)
